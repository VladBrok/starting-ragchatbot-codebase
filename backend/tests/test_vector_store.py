...
class TestVectorStore:
    ...
    def setup_method(self):
        ...
        with patch('vector_store.chromadb.PersistentClient') as mock_client:
            with patch('vector_store.chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction') as mock_embedding:
                ...
                self.vector_store = VectorStore(self.temp_dir, "test-model", max_results=5)
    ...
    def test_search_with_both_course_and_lesson_filters(self):
        ...
        # Act
        results = self.vector_store.search("query", course_name="Target", lesson_number=5)
    ...
    def test_add_course_metadata(self):
        ...
        course = Course(
            ...
            lessons=[
                Lesson(lesson_number=1, title="Lesson 1", lesson_link="http://example.com/lesson1"),
                Lesson(lesson_number=2, title="Lesson 2", lesson_link="http://example.com/lesson2")
            ]
        )
    ...
    def test_get_lesson_link_not_found(self):
        ...
        # Act
        link = self.vector_store.get_lesson_link("Test Course", 1)  # Looking for lesson 1, but only lesson 2 exists
